...
// --------------------------------------------------------------------- Small overlay window on a wlr layer surface: used by the notification daemon (notes, OSD) and by modules that need a popup (brightness slider).
struct PopupPlace {
    uint32_t anchor;
    int mt = 0, mr = 0, mb = 0, ml = 0;
};
// `avail` is the bar's along-axis length (screen width for a horizontal bar).
inline PopupPlace popup_place(double along, int size_along,
                              double avail = 0) {
    PopupPlace p{};
    int off = static_cast<int>(along + 0.5) - size_along / 2;
    // Keep the whole panel on screen: clamp the far edge first, then the near one — so an over-wide panel loses its far sid...
...
struct PopupWin {
    ...
    // `out` pins the popup to a specific monitor (nullptr = compositor picks).
    ...
    void ensure(Bar& b, uint32_t anchor, int mt, int mr, int mb, int ml,
        ...
        w = nw;
        h = nh;
        // Reopening on a different monitor needs a fresh surface: an output can only be chosen at layer-surface creation.
    ...
    void draw() {
        ...
        // HiDPI: buffer at the target output's scale, drawing code stays in logical coordinates (same scheme as BarSurface::draw).
        int sc = 1;
